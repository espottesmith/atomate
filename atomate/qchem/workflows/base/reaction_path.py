...
def get_workflow_reaction_path(molecule,
    ...
    Firework 1 : write QChem input for a frequency-flattening ts calculation
                 starting from some guess geometry,
                 run FF_opt QCJob,
                 parse directory, and insert into db
    Firework 2: Perturb the TS found in FW 1 along the imaginary frequency
                mode in the forwards direction,
                run FF_opt QCJob,
                parse directory, and insert into db
    Firework 3: Perturb the TS found in FW 1 along the imaginary frequency
